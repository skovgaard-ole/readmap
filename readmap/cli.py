...
The parser is generated from :data:`readmap.params.OPTIONS`, the same table the
...
change the meaning of the existing positional interface: ``readmap ref.gb
...
from readmap.params import OPTIONS, ExtractSpec, Params
...
_EPILOG = """\
examples:
  readmap ref.gb reads.btop
  readmap ref.fa reads.btop -r 4000 -m 1000 -x 20
      include reads over 4 kb, each match over 1 kb, stop at 20x coverage
  readmap ref.fa reads.btop -o result --identity
      set the output prefix and add the alignment-identity figure
  readmap ref.fa reads.btop -e reads.fastq,chr1,50000,100000
      also write out the reads covering chr1:50000-100000
  readmap ref.fa reads.btop -e reads.fastq.gz,chr1,50000,100000 -e reads.fastq.gz,chr2,25000,75000

Every run also writes <prefix>.readmapdb.npz, the cache the interactive browser
reads.  Open it with:
  readmap browse result.readmapdb.npz
...
examples:
  ...
  readmap browse ref.gb reads.btop -x 10
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="skip writing <prefix>.readmapdb.npz, the interactive browser's cache",
    )
    parser.add_argument(
...
def default_prefix(btop_file: str) -> str:
    ...
    while True:
        ...
        if dot and f".{suffix}".lower() in _STRIPPABLE:
            ...
            continue
        break
    return name or path.stem or "readmap"
...
class BrowseArgs:
    """A resolved ``readmap browse`` invocation.
...
#: Recognised by extension, so `browse something.readmapdb.npz` needs no flag.
CACHE_SUFFIX = ".readmapdb.npz"
...
def build_browse_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="readmap browse",
        description=(
            "Open an interactive genome browser over a readmap run: pan and "
