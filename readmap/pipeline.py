...
from readmap import extract, layout, report
from readmap import stats as stats_module
from readmap.classify import ClassifyStats, classify_stream
from readmap.inputs import btop as btop_reader
from readmap.inputs.reference import Reference, load_reference
from readmap.labels import LabelAssigner
from readmap.models import ReadClass, ReadGroup
from readmap.params import Params
from readmap.stats import Stats

_BANNER = (
    "\n" + "=" * 78 + "\n"
    "  readmap - visualising long sequence reads mapped by BLAST (BTOP)\n"
    + "=" * 78 + "\n"
)
...
def write_cache(analysis: Analysis, params: Params) -> Path | None:
    ...
    """
    from readmap.browser.store import build_store
...
def run(params: Params) -> int:
    ...
    if not params.no_pdf:
        ...
        from readmap.render import write_pdf
    ...
    if cache is not None:
        print(f"Wrote {cache}\n  Browse it with:  python -m readmap browse {cache}\n")
    else:
        print()
...
def browse(args) -> int:
    """Serve the interactive browser.  Takes a :class:`readmap.cli.BrowseArgs`."""
    from readmap.browser.server import serve
    from readmap.browser.store import BrowserStore, StoreFormatError
