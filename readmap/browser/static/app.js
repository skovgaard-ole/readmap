/* readmap browser -- interactive view over one cached run.
 *
 ...
 *
 * Colours come from /api/meta, which reads them from readmap/render/theme.py.
...
async function exportView(format) {
  ...
  try {
    ...
    link.href = url;
    link.download = (state.meta.sample || 'readmap') + '_' + state.contig.name +
...
async function boot() {
  try {
    ...
    document.body.innerHTML =
      '<p style="padding:24px;font-family:monospace">Cannot reach the readmap ' +
  ...
  document.title = 'readmap — ' + (meta.sample || 'browser');
