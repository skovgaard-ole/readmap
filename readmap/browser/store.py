...
definition :mod:`readmap.stats` uses -- so the track and the reported scalar
always agree.
"""
...
from readmap.models import Junction, ReadClass, ReadGroup
...
#: the metadata so a reader never has to import :mod:`readmap.models`.
...
def build_store(
    ...
    from readmap import __version__

    meta = {
        "format": FORMAT_VERSION,
        "readmap_version": __version__,
...
class StoreFormatError(Exception):
    """The file is not a readmap cache, or is one this build cannot read."""
...
class BrowserStore:
    ...
    def open(cls, path: str | Path) -> BrowserStore:
        ...
        if "meta" not in data:
            raise StoreFormatError(f"{target} is not a readmap cache")
        try:
