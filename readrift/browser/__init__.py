...
``readrift`` produces a poster: correct, complete, and 149 inches wide.  Finding
...
* :mod:`readrift.browser.store` -- a columnar cache of one analysed run.  Reading
...
* :mod:`readrift.browser.region` -- one visible window, packed into lanes.  Both
  the JSON API and the image exporter consume it, so they cannot disagree.
* :mod:`readrift.browser.server` -- a loopback-only HTTP server and the canvas
  front end in ``static/``.
...
def serve(*args, **kwargs):
    """Start the browser.  See :func:`readrift.browser.server.serve`."""
    from readrift.browser.server import serve as _serve
